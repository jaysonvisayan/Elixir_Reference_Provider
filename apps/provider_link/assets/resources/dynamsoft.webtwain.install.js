...
function OnWebTWAINModuleDownloadManually(objInstallerUrl, iPlatform, bSafari, bSSL){

	var ObjString = [
			'<div class="dynamsoft-dwt-dlg-title">',
			'Dynamic Web TWAIN Module<br />is not installed',
			'</div>',

			'<div style="margin:0 20px 20px 20px;text-align:center">',
			'<a id="dwt-btn-install" target="_blank" href="',
			objInstallerUrl['default'],
			'" onclick="DCP_DWT_onclickInstallButton(this)"><div class="dynamsoft-dwt-dlg-button">Start to Download</div></a>',
			'<div style="color:#AAAAAA">* Please manually install it</div></div>'
			];
	...
	if(iPlatform == EnumDWT_PlatformType.enumMac && bSafari && bSSL) {
		...
		ObjString.push('<div>');
		ObjString.push('If you still see the dialog after the installation,<br />');
		ObjString.push('please check <a href="https://developer.dynamsoft.com/dwt/dynamic-web-twain-module-is-not-installed" target="_blank">this article</a> for troubleshooting.');
		ObjString.push('</div>');
	}
